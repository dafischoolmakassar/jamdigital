...
| Auto-reload kiosk | Karena device TV pakai browser biasa (bukan Fully Kiosk Browser yang punya *Scheduled Restart* sendiri), `docs/state-machine.js` reload halaman sendiri tiap 10 menit — **tapi hanya saat state NORMAL**, supaya tidak memotong Adzan/Iqomah/Sholat yang sedang berjalan. Ini juga jadi cara perubahan dari Admin Panel (profil, jadwal, tema, slideshow) sampai ke layar TV tanpa perlu reload manual. |
